...
OVERVIEW:
We've defined image classes in the CSS named 'image-1' through 'image-6' that, when applied to a card, will make it show that particular image when it's flipped. Since the matching game works with pairs of images, we want to generate a random array with two of each image class strings (12 total).
...
function createCards(parentElement, shuffledImageClasses) {
  ...
  for (let i=0; i < 12; i++) {
    ...
    // Step 2(b): Add an image classe to the new card element using shuffledImageClasses[i].
    newElement.classList.add(shuffledImageClasses[i]);

    /* Step 2(c): Append a new object to array of card objects.  This should have properties for:
...
function onCardFlipped(newlyFlippedCard) {
  ...
  // Step 2: If this is the first card flipped then note that using the 'lastCardFlipped' variable and return (nothing else to do).
  ...
  if (!doCardsMatch(lastCardFlipped, newlyFlippedCard)) {
    // Second card flipped without match.
